...
Unreleased entries are not in this file. Each one is a separate file in the
[`.changelog/`](https://github.com/dendotai/stack/tree/dev/.changelog) directory
at the root of `dendotai/stack`, so two open pull requests never edit the same
line. A release moves every file into a new version section below and empties
the directory.
