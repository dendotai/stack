...
- **`Route` is a route file's only value export** (`apps/web`,
  ...
  -components/home.tsx, home.test.tsx}` and `routes/_app.tsx` is
  `routes/_app/{route.tsx, -components/app-layout.tsx}`; landing keeps a flat
  `index.tsx` with `routes/-components/landing.tsx`, and `__root.tsx` keeps its
  shell inline because every page renders it. `vite.config.ts` passes
...
- **Report template defects upstream** (`CLAUDE.md`): a new top section tells an
  ...
  code, to search `dendotai/stack` and file what it finds there, and to keep
  every private detail out of that issue. A defect found downstream is a defect
  in every project built from the template. To apply: copy the first section of
  `CLAUDE.md`.
